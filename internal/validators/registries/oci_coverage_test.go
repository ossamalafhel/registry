...
func TestValidateOCI_DockerHubAuth(t *testing.T) {
	...
	t.Run("Docker Hub successful auth", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			case r.URL.Path == "/v2/library/test-image/manifests/latest":
				// Check auth header
			...
			case r.URL.Path == "/v2/library/test-image/blobs/sha256:abc123":
				// Check auth header
				assert.Equal(t, "Bearer test-token", r.Header.Get("Authorization"))
...
func TestValidateOCI_MultiArchManifest(t *testing.T) {
	...
	t.Run("Multi-arch manifest", func(t *testing.T) {
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			case "/v2/test/repo/manifests/sha256:platform1":
